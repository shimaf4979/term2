...
public class MyPlayer extends ap25.Player {
    ...
    public Move think(Board board) {
        ...
        try {
            ...
            // 合法手チェック
            List<Move> legalMoves = myBoard.findLegalMoves(myColor);
            ...
            // 時間配分
            long allocatedTime = timeManager.allocateTime(myBoard, remainingTime, moveCount);

            // 即座勝利チェック
            // Move winningMove = findWinningMove(myBoard, myColor, legalMoves);
            // if (winningMove != null) {
            ...
            if (emptyCount <= 6) {
                ...
                Move perfectMove = endgameSolver.solve(myBoard, myColor, Math.min(allocatedTime, 5000));
                if (perfectMove != null) {
                    return perfectMove;
                }
            }
            ...
            // メイン探索
            return performIterativeDeepening(myBoard, myColor, allocatedTime);
     ...
     * 反復深化による探索（異常値検出強化版）
     */
    private Move performIterativeDeepening(MyBoard board, Color color, long timeLimit) {
     ...
     * 即座勝利手の検索
     */
    private Move findWinningMove(MyBoard board, Color color, List<Move> moves) {
        for (Move move : moves) {
...
class PVSEngine {
     ...
     * ルート探索
     */
    public PVSResult searchRoot(MyBoard board, Color color, int depth,
     ...
     * PVS探索
     */
    private PVSResult pvs(MyBoard board, Color color, int depth, int alpha, int beta,
...
class TranspositionTable {
    ...
    public TTEntry probe(MyBoard board, int depth, int alpha, int beta) {
    ...
    public void store(MyBoard board, Move bestMove, int score, int depth, int flag) {
        long hash = calculateHash(board);
        int index = (int) (hash & TABLE_MASK);
...
class Evaluator {
    ...
    public int evaluate(MyBoard board, Color color) {
        int score = 0;
    ...
    public int evaluateTerminal(MyBoard board, Color color) {
        int stoneDiff = board.count(color) - board.count(color.flipped());
        if (stoneDiff > 0) {
    ...
    private int evaluateStability(MyBoard board, Color color) {
        int stability = 0;
...
class MoveOrderer {
    ...
    public List<Move> orderMoves(List<Move> moves, MyBoard board, Color color, List<Move> pv) {
...
class EndgameSolver {
    ...
    public Move solve(MyBoard board, Color color, long timeLimit) {
        long deadline = System.currentTimeMillis() + timeLimit;
    ...
    private int solveRecursiveSafe(MyBoard board, Color color, long deadline, int depth) {
        // 安全装置: 深度制限
    ...
    private long calculateHash(MyBoard board, Color color) {
        long hash = 0;
        for (int i = 0; i < LENGTH; i++) {
            Color c = board.get(i);
 ...
 */
class TimeManager {
    public long allocateTime(MyBoard board, long remainingTime, int moveCount) {
        int emptyCount = countEmpty(board);
    ...
    private double calculateImportance(MyBoard board, int emptyCount, int moveCount) {
